...
const DiscountInlineEditor: React.FC<DiscountInlineEditorProps> = ({
  ...
  const [range, setRange] = useState<[Dayjs, Dayjs] | undefined>(
    initialValue.discountRange,
  );

  return (
    <div className="flex flex-col gap-2 p-2 bg-white border border-gray-200 shadow-lg rounded-lg z-50 absolute right-0 min-w-70">
      ...
      {hasDiscount && (
        <>
          ...
          <ConfigProvider
            theme={{ components: { DatePicker: { cellHeight: 20 } } }}
          >
            <RangePicker
              size="small"
              value={range}
