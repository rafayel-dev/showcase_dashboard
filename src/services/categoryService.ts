...
const generateCategoryId = (): string => {
    return `CAT${Math.max(..._categories.map(c => parseInt(c.id.replace('CAT', '')))) + 1}`.padStart(6, '0');
};
...
export const addCategory = (category: Omit<Category, 'id' | 'key'>): Promise<Category> => {
  return new Promise((resolve) => {
    setTimeout(() => {
      const newCategory = { ...category, id: generateCategoryId(), key: generateCategoryId() };
